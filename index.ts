...
function* dataGenerator(to: number) {
    let from = 1
    while (from <= to) yield { number: from, filename: `data-${from++}.json` }
}

function* filenameGenerator(to: number) {
    let from = 1
    while (from <= to) yield `${testDataDir}/data-${from++}.json`
}
...
async function generateData() {
    await new Promise<void>((resolve, reject) => {
        ...
        fs.mkdir(testDataDir, { recursive: true }, (err) => {
            if (err)
                throw Error(
                    `Fail to create directory ${testDataDir}: ${err?.message}`,
                )
        })
    })
    await Promise.all<void>(
        Array.from(dataGenerator(size)).map((data) => new Promise((resolve, reject) => {
                fs.writeFile(
                    `${testDataDir}/${data.filename}`,
                    JSON.stringify(data),
                    (err) => {
                        if (err) reject(err)
                        else resolve()
...
async function readData(filename: string, logger: BufferdLogger) {
